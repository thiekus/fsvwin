...
#include "gtkdateedit.h"
#include <glib-object.h>

struct _GtkDateEditPrivate {
...
static void gtk_date_edit_destroy      (GtkObject          *object);
static void gtk_date_edit_finalize     (GObject            *object);
static void gtk_date_edit_set_property    (GObject            *object,
					  guint               param_id,
					  const GValue       *value,
					  GParamSpec         *pspec);
static void gtk_date_edit_get_property    (GObject            *object,
					  guint               param_id,
					  GValue             *value,
					  GParamSpec         *pspec);

static void create_children (GtkDateEdit *gde);
 ...
 *
 * Returns the GType for the GtkDateEdit widget
 */
/* The following macro defines the get_type */

G_DEFINE_TYPE (GtkDateEdit, gtk_date_edit, GTK_TYPE_HBOX);

static void 
gtk_date_edit_init(GtkDateEdit *gde){

}

GType
gtk_date_edit_flags_get_type (void)
{
  static GType type = 0;

  if (G_UNLIKELY (type == 0))
  {
    static const GFlagsValue _gtk_date_edit_flags_values[] = {
      { GTK_DATE_EDIT_SHOW_TIME, "GTK_DATE_EDIT_SHOW_TIME", "show-time" },
      { GTK_DATE_EDIT_24_HR, "GTK_DATE_EDIT_24_HR", "24-hr" },
      { GTK_DATE_EDIT_WEEK_STARTS_ON_MONDAY, "GTK_DATE_EDIT_WEEK_STARTS_ON_MONDAY", "week-starts-on-monday" },
      { GTK_DATE_EDIT_DISPLAY_SECONDS, "GTK_DATE_EDIT_DISPLAY_SECONDS", "display-seconds" },
      { 0, NULL, NULL }
    };

    type = g_flags_register_static ("GtkDateEditFlags", _gtk_date_edit_flags_values);
  }
...
static void
day_selected (GtkCalendar *calendar, GtkDateEdit *gde)
{
	char buffer [256];
...
static void
day_selected_double_click (GtkCalendar *calendar, GtkDateEdit *gde)
{
	hide_popup (gde);
}
...
static void
position_popup (GtkDateEdit *gde)
{
	gint x, y;
	gint bwidth, bheight;
	GtkRequisition req;
...
static void
select_clicked (GtkWidget *widget, GtkDateEdit *gde)
{
	const char *str;
	GDate *date;
	int month;
...
static void
fill_time_popup (GtkWidget *widget, GtkDateEdit *gde)
{
	GtkWidget *menu;
	struct tm *mtm;
	time_t current_time;
	...
	for (i = gde->_priv->lower_hour; i <= gde->_priv->upper_hour; i++){
		...
		if (gde->_priv->flags & GTK_DATE_EDIT_24_HR) {
			if (strftime (buffer, sizeof (buffer),
				      "%H:00", mtm) == 0)
				strcpy (buffer, "???");
		} else {
...
static gboolean
gtk_date_edit_mnemonic_activate (GtkWidget *widget,
				   gboolean   group_cycling)
{
	gboolean handled;
	GtkDateEdit *gde;
	
	gde = GTK_DATE_EDIT (widget);
...
static void
gtk_date_edit_class_init (GtkDateEditClass *class)
{
	...
	object_class->destroy = gtk_date_edit_destroy;

	gobject_class->finalize = gtk_date_edit_finalize;
	gobject_class->get_property = gtk_date_edit_get_property;
	gobject_class->set_property = gtk_date_edit_set_property;

	widget_class->mnemonic_activate = gtk_date_edit_mnemonic_activate;
	...
	g_object_class_install_property (gobject_class,
					 ...
					 g_param_spec_flags ("dateedit_flags",
							     ...
							     _("Flags for how "
							       "DateEdit looks"),
							     GTK_TYPE_DATE_EDIT_FLAGS,
							     GTK_DATE_EDIT_SHOW_TIME,
							     (G_PARAM_READABLE |
							      G_PARAM_WRITABLE)));
	g_object_class_install_property (gobject_class,
					 PROP_LOWER_HOUR,
	...
	date_edit_signals [TIME_CHANGED] =
		g_signal_new ("time_changed",
			      ...
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (GtkDateEditClass, time_changed),
			      NULL, NULL,
	...
	date_edit_signals [DATE_CHANGED] =
		g_signal_new ("date_changed",
			      G_TYPE_FROM_CLASS (gobject_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (GtkDateEditClass, date_changed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE, 0);
...
static void
gtk_date_edit_instance_init (GtkDateEdit *gde)
{
	gde->_priv = g_new0(GtkDateEditPrivate, 1);
	gde->_priv->lower_hour = 7;
	gde->_priv->upper_hour = 19;
	gde->_priv->flags = GTK_DATE_EDIT_SHOW_TIME;
	create_children (gde);
}

static void
gtk_date_edit_destroy (GtkObject *object)
{
	GtkDateEdit *gde;
	...
	g_return_if_fail (GTK_IS_DATE_EDIT (object));

	gde = GTK_DATE_EDIT (object);

	if(gde->_priv->cal_popup)
		gtk_widget_destroy (gde->_priv->cal_popup);
	gde->_priv->cal_popup = NULL;

//	GTK_CALL_PARENT (GTK_OBJECT_CLASS, destroy, (object));
}

static void
gtk_date_edit_finalize (GObject *object)
{
	GtkDateEdit *gde;
...
//	GTK_CALL_PARENT (G_OBJECT_CLASS, finalize, (object));
}

static void
gtk_date_edit_set_property (GObject            *object,
			   guint               param_id,
			   const GValue       *value,
			   GParamSpec         *pspec)
{
	...
	self = GTK_DATE_EDIT (object);

	switch (param_id) {
	case PROP_TIME:
		gtk_date_edit_set_time(self, g_value_get_ulong (value));
		break;
	case PROP_DATEEDIT_FLAGS:
		gtk_date_edit_set_flags(self, g_value_get_flags (value));
		break;
	case PROP_LOWER_HOUR:
		gtk_date_edit_set_popup_range(self, g_value_get_int (value),
						self->_priv->upper_hour);
		break;
	case PROP_UPPER_HOUR:
		gtk_date_edit_set_popup_range(self, self->_priv->lower_hour,
...
static void
gtk_date_edit_get_property (GObject            *object,
			   guint               param_id,
			   GValue             *value,
			   GParamSpec         *pspec)
{
...
/**
 * gtk_date_edit_set_time:
 * @gde: the GtkDateEdit widget
 * @the_time: The time and date that should be set on the widget
 *
 * Description:  Changes the displayed date and time in the GtkDateEdit
 * widget to be the one represented by @the_time.  If @the_time is 0
 * then current time is used.
 */
void
gtk_date_edit_set_time (GtkDateEdit *gde, time_t the_time)
{
	struct tm *mytm;
	char buffer [256];
	char *str_utf8;
	...
	/* Set the time */
	if (gde->_priv->flags & GTK_DATE_EDIT_24_HR) {
		if (gde->_priv->flags & GTK_DATE_EDIT_DISPLAY_SECONDS) {
 ...
 */
void
gtk_date_edit_set_popup_range (GtkDateEdit *gde, int low_hour, int up_hour)
{
        g_return_if_fail (gde != NULL);
...
static void
create_children (GtkDateEdit *gde)
{
	GtkWidget *frame;
	GtkWidget *hbox;
	GtkWidget *arrow;
	...
	if (gde->_priv->flags & GTK_DATE_EDIT_SHOW_TIME)
	 ...
	 * future call to gtk_date_edit_set_popup_range
	 */
	...
	gtk_calendar_display_options (GTK_CALENDAR (gde->_priv->calendar),
				      (GTK_CALENDAR_SHOW_DAY_NAMES
				       ...
				       | ((gde->_priv->flags & GTK_DATE_EDIT_WEEK_STARTS_ON_MONDAY)
 ...
 *
 * Description: Creates a new #GtkDateEdit widget which can be used
 * to provide an easy to use way for entering dates and times.
 * If @the_time is 0 then current time is used.
 *
 * Returns: a new #GtkDateEdit widget.
 */
GtkWidget *
gtk_date_edit_new (time_t the_time, gboolean show_time, gboolean use_24_format)
{
	return gtk_date_edit_new_flags (the_time,
					  ((show_time ? GTK_DATE_EDIT_SHOW_TIME : 0)
					   | (use_24_format ? GTK_DATE_EDIT_24_HR : 0)));
}

/**
 * gtk_date_edit_new_flags:
 * @the_time: The initial time for the date editor.
 * @flags: A bitmask of GtkDateEditFlags values.
 *
 * Description:  Creates a new #GtkDateEdit widget with the
 * specified flags. If @the_time is 0 then current time is used.
 *
 * Returns: the newly-created date editor widget.
 **/
GtkWidget *
gtk_date_edit_new_flags (time_t the_time, GtkDateEditFlags flags)
{
	GtkDateEdit *gde;

	gde = g_object_new (GTK_TYPE_DATE_EDIT, NULL);

	gtk_date_edit_construct(gde, the_time, flags);

	return GTK_WIDGET (gde);
}

/**
 * gtk_date_edit_construct:
 * @gde: The #GtkDateEdit object to construct
 * @the_time: The initial time for the date editor.
 * @flags: A bitmask of GtkDateEditFlags values.
 *
 ...
 **/
void
gtk_date_edit_construct (GtkDateEdit *gde, time_t the_time, GtkDateEditFlags flags)
{
	gtk_date_edit_set_flags (gde, flags);
	gtk_date_edit_set_time (gde, the_time);
}

/**
 * gtk_date_edit_get_time:
 * @gde: The GtkDateEdit widget
 *
 * Returns the time entered in the GtkDateEdit widget
 */
time_t
gtk_date_edit_get_time (GtkDateEdit *gde)
{
	struct tm tm = {0};
	const char *str;
	GDate *date;
...
#ifndef GTK_DISABLE_DEPRECATED_SOURCE

/**
 * gtk_date_edit_get_date:
 * @gde: The GtkDateEdit widget
 *
 * Deprecated, use #gtk_date_edit_get_time
 *
 * Returns:
 */
time_t
gtk_date_edit_get_date (GtkDateEdit *gde)
{
	g_warning(_("gtk_date_edit_get_date deprecated, use gtk_date_edit_get_time"));
	return gtk_date_edit_get_time(gde);
}

#endif /* not GTK_DISABLE_DEPRECATED_SOURCE */


/**
 * gtk_date_edit_set_flags:
 * @gde: The date editor widget whose flags should be changed.
 * @flags: The new bitmask of GtkDateEditFlags values.
 *
 * Changes the display flags on an existing date editor widget.
 **/
void
gtk_date_edit_set_flags (GtkDateEdit *gde, GtkDateEditFlags flags)
{
        GtkDateEditFlags old_flags;

	g_return_if_fail (gde != NULL);
	g_return_if_fail (GTK_IS_DATE_EDIT (gde));

        old_flags = gde->_priv->flags;
        gde->_priv->flags = flags;

	if ((flags & GTK_DATE_EDIT_SHOW_TIME) != (old_flags & GTK_DATE_EDIT_SHOW_TIME)) {
		if (flags & GTK_DATE_EDIT_SHOW_TIME) {
			gtk_widget_show (gde->_priv->cal_label);
			gtk_widget_show (gde->_priv->time_entry);
			gtk_widget_show (gde->_priv->time_popup);
		} else {
	...
	if ((flags & GTK_DATE_EDIT_24_HR) != (old_flags & GTK_DATE_EDIT_24_HR))
		fill_time_popup (GTK_WIDGET (gde), gde); /* This will destroy the old menu properly */

	if ((flags & GTK_DATE_EDIT_WEEK_STARTS_ON_MONDAY)
	    != (old_flags & GTK_DATE_EDIT_WEEK_STARTS_ON_MONDAY)) {
		if (flags & GTK_DATE_EDIT_WEEK_STARTS_ON_MONDAY)
			gtk_calendar_display_options (GTK_CALENDAR (gde->_priv->calendar),
